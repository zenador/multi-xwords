...
oyCrosswordPuzzle.prototype.init = function(){ 
	...
	target.innerHTML = trackAction + '<a id="oygHeaderMenuBtn" href=""></a>'; // removed the ugly image link outside, but didn't remove this whole paragraph and the next because that causes the game to screw up when it talks to the server, maybe because of trackAction
...
oyCrosswordPuzzle.prototype.bindItem = function(x, y){	
	...
	if (target != null){
		...
		target.onkeyup = function(e){ 
			...
			//document.getElementById("oygStatic").innerHTML = c;
			//document.getElementById("oygStatic").innerHTML += target.value;
...
oyCrosswordPuzzle.prototype.handleKeyPress = function(x, y, e){
	...
	if (this.isValidChar(c)){ 
		var dumbff = (keyCode >= 37 && keyCode <= 40) || keyCode == 8 || keyCode == 9 || keyCode == 46;
		if (!dumbff) { // necessary because firefox inteprets dir keys, backspaces, tabs and delkeys as keypresses, unlike normal browsers which only treat them as keydowns
			var target = this.inputCache.getElement(x, y);
			if (!target.readOnly && keyCode != 13){ // 13 is enter, to make sure that pressing enter moves on to the next cell without changing contents of current cell
				target.value = c;
			}
...
oyCrosswordPuzzle.prototype.handleKeyUp = function(x, y, e){
	...
	if(target.value.length >= 1) {
		var nonimekey = (keyCode >= 65 && keyCode <= 90) || (keyCode >= 48 && keyCode <= 57) || (keyCode >= 37 && keyCode <= 40) || keyCode == 8 || keyCode == 46 || keyCode == 16 || keyCode == 20 || keyCode == 222 || keyCode == 192;
		if (!nonimekey) {
			this.moveToNextCell(x, y);
		}
	}
...
oyCrosswordPuzzle.prototype.handleKeyDown = function(x, y, e){
	...
	var dir = (keyCode >= 37 && keyCode <= 40) || keyCode == 8 || keyCode == 46 || keyCode == 9;	
	if (dir) {
		var target = null;
		...
		switch(keyCode){
			case 8: //backspace
				e.preventDefault();
				if (!this.inputCache.getElement(x, y).readOnly){
